...
def _inject_through_recovery(prep, cls: str, target_snr: float, n_seg: int, dt: float,
                             flow: float, fmax: float) -> tuple[np.ndarray, float]:
    """Build the recovery-matched injection and return (time-domain trace, exact SNR).

    The injection is generated by the SAME waveform + detector the recovery uses --
    the CCSN through the antenna response (prep.detectors[0].fd_response), the blip
    through a no-response detector -- so injection and template are consistent. The
    optimal SNR is computed in the likelihood's exact convention: ``<h|h>`` over the
    analysis band against the prepared (line-notched) ``det.psd``. log_amp is then
    scaled so the injected SNR equals ``target_snr`` by construction.
    """
    sr = 1.0 / dt
    ...
    if cls == "signal":
        wf = StarccatoJimWaveform(model=get_model("ccsne"), sample_rate=sr, window=prep.window)
        det = prep.detectors[0]  # full antenna response
    else:
        wf = StarccatoGlitchWaveform(model=get_model("blip"), sample_rate=sr,
                                     window=prep.window, strain_scale=1e-21)
        det = _clone_no_response_detector(prep.detectors[0])  # incoherent, no response
    psd_full = np.asarray(det.psd.values)
    band = (full_freqs >= flow) & (full_freqs <= fmax)

    params = {name: 0.0 for name in wf.latent_names}
    params.update({"log_amp": 0.0, "t_c": 0.0, "ra": 0.0, "dec": 0.0, "psi": 0.0,
                   "luminosity_distance": 10.0, "gmst": float(prep.gmst),
                   "trigger_time": float(prep.trigger_time)})

    def _hdec(p):
        h_sky = wf(fj, p)
        return np.asarray(det.fd_response(fj, h_sky, p))

    # Unit-amplitude SNR in the likelihood convention, then scale log_amp to target.
    hdec0 = _hdec(params)
...
def run_event(
    ...
    *,
    ...
    noise = simulate_noise_fd(psd_vals, n_seg, dt, seed)
    ...
    if cls in ("signal", "glitch"):
        ...
        prep = prepare_multi_detector_data(["L1"], bundle_paths={"L1": noise_bundle},
                                           flow=flow, fmax=fmax)
        inj_td, injected_snr = _inject_through_recovery(prep, cls, target_snr, n_seg, dt, flow, fmax)
        noise = noise + inj_td
