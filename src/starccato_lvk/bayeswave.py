    ...
    for item in inputs:
        ...
        if overwrite or not item.frame.is_file():
            ...
            with h5py.File(item.bundle, "r") as h5:
                ...
                analysis_values = np.asarray(h5["strain/values"], dtype=float)
            if values.size != expected or not np.all(np.isfinite(values)):
                raise ValueError(
                    f"bundle {dataset} is incomplete or non-finite: {item.bundle}"
                )
            if analysis_values.size != item.source_samples or not np.all(
                np.isfinite(analysis_values)
            ):
                raise ValueError(
                    f"bundle strain/values is incomplete or non-finite: {item.bundle}"
                )
            if item.has_offsource_psd:
                start = int(round((item.t0 - item.frame_t0) / item.source_dt))
                stop = start + item.source_samples
                if start < 0 or stop > values.size:
                    raise ValueError(
                        f"analysis segment lies outside full_strain: {item.bundle}"
                    )
                # The acquisition bundle stores injections only in strain/values;
                # full_strain remains the unmodified off-source acquisition
                # stretch. Overlay the ranked segment so matching the PSD window
                # does not silently remove an injection from the GWF.
                values[start:stop] = analysis_values
            series = TimeSeries(
                values,
                t0=item.frame_t0,
                dt=item.source_dt,
